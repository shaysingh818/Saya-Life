...
func AddClient(Address, MacAddress string, UnitId int, Online bool) Gateway{
	channel := make(chan string)
	instance := Gateway{Address, UnitId, channel, Online, MacAddress}
	return instance
}
...
func PushAll(w http.ResponseWriter, req *http.Request){
	for _, value := range clients{
		...
		if(resp.StatusCode == 200){
			...
			fmt.Println("Error Pushing:", value.MacAddr)
		}
	}
...
func pollResponse(w http.ResponseWriter, r *http.Request){
	...
	createClient := AddClient(Addr,MacAddr, ClientCount, true)
